...
<body>
  ...
  <script>
    const colors = {
      principal: '#4f46e5', group: '#059669', service: '#0ea5e9', finding: '#f97316',
      share: '#a855f7', secret: '#ef4444', domain: '#facc15', default: '#64748b'
    };
    const byTypeColor = t => colors[t] || colors.default;
    let payload;
    let fullData;
    ...
    function applyFilters() {
      ...
      const links = fullData.links.filter(l => (!ef || l.type === ef) && nodeSet.has(l.source) && nodeSet.has(l.target));
    ...
    function focusNode(node) {
      ...
      list.innerHTML = '';
      const rels = fullData.links.filter(l => l.source === node.id || l.target === node.id);
      rels.forEach(r => {
        const item = document.createElement('div');
        item.textContent = r.source + ' --(' + r.type + '/' + r.validation + ')-> ' + r.target;
